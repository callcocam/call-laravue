 ...
 * the `classKeys` global option when registering form.
 */
...
/**
 * This function is responsible for providing Vueform’s default classes.
...
const classGenerator = (classKey, context) => {
  ...
  const base = `form${prefix}${element}${key !== 'outer' ? `-${key}` : ''}`
...
const classModifiers = (base, classKey, context) => {
  ...
  switch (classKey) {
    ...
    case 'element':
      ...
      if (type === 'group') {
        modifiers.push('form-input-group')
      }
      break
    case 'help':
...
export const classProps = (() => {
  const stateKeys = [''].concat(Object.keys(states).map(s => capitalize(s)))
...
export function applyStates (elementKey, baseClass, globals, context) {
  return Object.keys(states).reduce((classes, stateKey) => {
    ...
    if (states[stateKey](context)) {
      const key = `${elementKey}${capitalize(stateKey)}`
