<template>
    <div>
        <button @click="showModal = true" type="button">
            <x-icon name="fa-trash" class="text-base w-5 h-5" />
        </button>
                        ...
                        <div class="flex min-h-full items-center justify-center p-4 text-center ">
                            <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                                ...
                                <DialogPanel
                                    ...
                                    <DialogTitle as="div"
                                        class="text-base font-medium text-slate-700 dark:text-navy-100 py-2">
                                        <h2 class="text-2xl px-3 text-red-500">Atenção</h2>
                                        <p class="text-lg ">Tem certeza de que deseja excluir a <b>{{  $t('BOARD') }}</b>?</p>
                                    </DialogTitle>
...
const apply = async () => {
    try {
        const { data } = await MAKEAPP.delete(('make/boards/').concat(form.value.id))
        const { message } = data
        if (message) {
