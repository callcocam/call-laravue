import { setId, token } from './helpers'
...
class Upload {
    ...
    getUploader(...args) {
        if (this.uploaderIsAxios()) {
            ...
            if (this.context.uploadUrl === false) {
                throw new Error('No uploadURL specified')
            }
    ...
    removeFile(uuid) {
        ...
        if (window && this.fileList instanceof FileList && this.supportsDataTransfers) {
            ...
            this.options.form.file = new Upload(this.input, this.context, this.globalOptions)
        } else {
    ...
    toString() {
        ...
        return this.results ? JSON.stringify(this.results, null, '  ') : `Upload(${descriptor})`
    }
